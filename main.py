...
# ==============================================================================
print("🚀 Iniciando Motor de Inteligência de Carteira V2 (Blindado)...")
...
# ==============================================================================
# 2. FUNÇÕES ROBUSTAS DE TRATAMENTO DE DADOS (RESGATADAS DO SEU CÓDIGO)
# ==============================================================================
...
def garantir_inteiro(x):
    try:
        ...
        val_str = str(x).replace('R$', '').replace('.', '').replace(',', '.').strip()
        if val_str in ['', '-']: return 0
...
# ==============================================================================
# 3. CARREGAMENTO E HIGIENIZAÇÃO DE DADOS
# ==============================================================================
print("📊 Carregando e limpando dados...")
df_mov = pd.DataFrame(aba_mov.get_all_records(value_render_option='UNFORMATTED_VALUE'))
df_mov['Data'] = df_mov.get('Data', pd.Series()).apply(converter_data)
df_mov['Ticker'] = df_mov.get('Ticker', '').astype(str).str.strip().str.upper()

# ==============================================================================
# 4. MOTOR DE PREÇO MÉDIO E CONSTRUÇÃO DE PORTFÓLIO
# ==============================================================================
print("⚙️ Calculando Preço Médio e Posições (Algoritmo FIFO)...")
...
for idx, row in df_mov_calc.iterrows():
    ...
    if mov in ['Rendimento', 'Juros Sobre Capital Próprio', 'Jcp', 'Dividendo']:
        proventos.append({
            'Data': row['Data'], 
            'AnoMes': row['Data'].strftime('%Y-%m') if pd.notna(row['Data']) else 'N/D',
    ...
    if mov == 'Compra' and es == 'Credito':
        ...
        if valor_op > 0:
            ...
            if tk_antigo and tk_antigo in carteira:
                ...
                carteira[tk_antigo]['custo'] = carteira[tk_antigo]['qtd'] * carteira[tk_antigo]['pm'] if carteira[tk_antigo]['qtd'] > 0 else 0.0
...
# ==============================================================================
# 5. ATUALIZAÇÃO DA PLANILHA (INF_ATIVOS E CARTEIRA)
# ==============================================================================
...
mapa_tipos = dict(zip(df_inf_sheet.iloc[:, 0].astype(str), df_inf_sheet.iloc[:, 2])) if not df_inf_sheet.empty and len(df_inf_sheet.columns) > 2 else {}
mapa_segs = dict(zip(df_inf_sheet.iloc[:, 0].astype(str), df_inf_sheet.iloc[:, 3])) if not df_inf_sheet.empty and len(df_inf_sheet.columns) > 3 else {}
...
# ==============================================================================
# 6. INTELIGÊNCIA E CÁLCULO DE INSIGHTS GLOBAIS
# ==============================================================================
print("🧠 Processando Insights de Investimento (Aguardando Fórmulas 5s)...")
time.sleep(5) 
df_res_atualizado = pd.DataFrame(aba_carteira.get_all_records(value_render_option='UNFORMATTED_VALUE'))

total_investido = garantir_valor_financeiro(df_res_atualizado['Investido'].sum()) if not df_res_atualizado.empty else 0.0
total_patrimonio = garantir_valor_financeiro(df_res_atualizado['Patrimônio'].sum()) if not df_res_atualizado.empty else 0.0
total_proventos = garantir_valor_financeiro(df_prov['Valor'].sum()) if not df_prov.empty else 0.0
...
prov_12m = garantir_valor_financeiro(df_prov_12m['Valor'].sum()) if not df_prov_12m.empty else 0.0
...
if yoc_12m > 6.0: insights.append(f"🔥 Excelente! Seu *Yield on Cost* (12M) está em **{yoc_12m:.2f}%**, gerando forte renda passiva.")
else: insights.append(f"💡 Seu *Yield on Cost* (12M) é de **{yoc_12m:.2f}%**. Continue aportando para acelerar seus rendimentos.")

if not df_res_atualizado.empty:
    try:
        campeao = df_res_atualizado.loc[pd.to_numeric(df_res_atualizado['Variação'], errors='coerce').idxmax()]
        insights.append(f"🏆 O ativo campeão da carteira hoje é **{campeao['Ticker']}**, com **{campeao['Variação']*100:.2f}%** de lucro.")
    except: pass

# ==============================================================================
# 7. GERAÇÃO DO DASHBOARD PREMIUM (HTML/JS)
# ==============================================================================
print("🎨 Gerando Dashboard Premium...")

js_carteira = df_res_atualizado.to_json(orient='records') if not df_res_atualizado.empty else "[]"
...
<body>
    ...
    <div id="resumo" class="dashboard-container active">
        ...
        <div class="kpi-grid">
            ...
            <div class="card">
                ...
                <div class="kpi-value" style="__COR_TEXT_LUCRO__">__LUCRO_TOTAL__</div>
            ...
            <div class="card">
                ...
                <div class="kpi-value" style="color: var(--primary);">__YOC_12M__%</div>
    ...
    <script>
        ...
        let setores = {};
        carteira.forEach(c => { 
            let valor = typeof c['Patrimônio'] === 'number' ? c['Patrimônio'] : parseFloat(String(c['Patrimônio']).replace(',','.'));
            if (!isNaN(valor)) { setores[c.Seguimento] = (setores[c.Seguimento] || 0) + valor; }
        });
...
print("✅ Sucesso! Dashboard 'index.html' gerado com segurança e protegido contra erros.")
